...
(function(){
    ...
    var re_get_style = /([^\0]*?)<\x2fstyle>/gi;
    ...
    function END_ANGLE(txt) {
        ...
        if (m) {
            ...
            var tok = [{ content : m[0],
                     style   : "paren xml-tagangle" }];
            if (/^style$/i.test(tag) && !/><\x2style>/i.test(txt)) {
                re_get_style.lastIndex = 1;
                var n = re_get_style.exec(txt);
                if (n && n[1] && n.index ==1){
                    var code = n[1];
                    var index = re_get_style.lastIndex - 10;
                    var css = new H({ lang: "css",
                              noTrim: true }).doItNow(code);
                    var csstok = {
                        content : { escaped: css },
                        style   : "xml-inline-style",
                        index   : index
                    };
                    tok.push(csstok);
                }
