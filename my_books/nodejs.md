...
```
* 9.3 集群稳定之路
  * 3.1 进程事件 - 子进程 error, exit, close, disconnect 相关事件；
  * 3.2 自动重启 - 通过件监听 父子进程 相关事件， 实现自动重启；
  ```
  自杀信号 - 在退出的流程中增加一个自杀（suicide）信号。工作进程在得知要退出时，向主进程发送一个自杀信号，然后才停止接收新的连接，当所有连接断开后才退出。
  限量重启 - 在退出的流程中增加一个自杀（suicide）信号。工作进程在得知要退出时，向主进程发送一个自杀信号，然后才停止接收新的连接，当所有连接断开后才退出。设置 重启上限值， 避免无限重启。
  ```
  * 3.3 负载均衡
  * 负载均衡策略：
  ```
  ...
  ```
  * 3.4 状态共享
  * 第三方存储 保存共享数据（eg: Macache、Redis）

* 9.4 Cluster 模块
* cluster模块，用以解决多核CPU的利用率问题，同时也提供了较完善的API，用以处理进程的健壮性问题。
 * 4.1 Cluster 工作原理：子进程 与 网络模块(Net 模块)结合实现；
 * 4.2 Cluster 事件： fork（复制）、online（连接）、listening（监听）、disconnect（断开连接）、exit（退出）、setup（启动）

* 9.5 总结
参考Unix的设计理念， 开启多管理进程 + 任务子进程 实现 NodeJs 负载均衡等性能优化
```
通过简单的主从模式, 启动很多子进程来处理任务，结构甚至远比主从模式复杂，但是每个子进程应当是简单到只做好一件事，然后通过进程间通信技术将它们连接起来即可。
通过child_process模块可以大幅提升Node的稳定性，但是一旦主进程出现问题，所有子进程将会失去管理。在Node的进程管理之外，还需要用监听进程数量或监听日志的方式确保整个系统的稳定性，即使主进程出错退出，也能及时得到监控警报，使得开发者可以及时处理故障。
```


## 第十章 测试
* 测试的意义在于，在用户消费产出的代码之前，开发者首先消费它，给予其重要的质量保证。测试包含单元测试、性能测试、安全测试和功能测试等几个方面，本章将从Node实践的角度来介绍单元测试和性能测试。

* 10.1 单元测试
  * 单元测试原则： 1）单一职责； 2）接口抽象； 3）层次分离；
  * 单元测试介绍： 1）断言； 2）测试框架（mocha）； 3）测试报告； 4）测试用例；5）测试覆盖率；6）mock (模拟异常)； 7）私有方法测试；
  * 工程化 与 自动化： 1）工程化:工具：Makefile; 2）持续集成： 利用travis-ci实现持续集成；

* 10.2 性能测试
  * 单元测试主要用于检测代码的行为是否符合预期。在完成代码的行为检测后，还需要对已有代码的性能作出评估，检测已有功能是否能满足生产环境的性能要求，能否承担实际业务带来的压力。
  * 基准测试， 基准测试要统计的就是在多少时间内执行了多少次某个方法。 可用 benchmark这个模块 实现；
  * 压力测试， 通常会对网络接口进行压力测试以判断网络接口的性能， 最常用的工具是ab、siege、http_load等；
  * 基准测试驱动开发；1）写基准测试；2）写/改 代码；3）收集数据；4）找出问题；5）返回第2）步。
  * 测试数据域业务数据转换， 如果用户量只有几个，每天的PV只有几十个，那么网站开发几乎不需要什么优化就能胜任。如果PV上10万甚至百万、千万，就需要运用性能测试来验证是否能够满足实际业务需求，如果不满足，就要运用各种优化手段提升服务能力。

* 10.3 总结
  * 测试是应用或者系统最重要的质量保证手段。有单元测试实践的项目，必然对代码的粒度和层次都掌握得较好。单元测试能够保证项目每个局部的正确性，也能够在项目迭代过程中很好地监督和反馈迭代质量。如果没有单元测试，就如同黑夜里没有秉烛的行走。对于性能，在编码过程中一定存在部分感性认知，与实际情况有部分偏差，而性能测试则能很好地斧正这种差异。